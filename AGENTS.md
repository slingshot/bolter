...
- **Completion is one-shot**: `/upload/complete` treats a stored `auth` field as "already completed" — file IDs become public once a link is shared, so re-completion must never overwrite auth or metadata. A retry carrying the same `authKey` (uploader recovering from a lost response) returns success idempotently; anything else gets `401`. Cancelled uploads abort the server-side multipart (S3 parts + Redis metadata) even when the cancel surfaces as a thrown error, and `/upload/abort` cleans up via `storage.del` so the provider file counter is decremented.
...
- **Completeness verification**: Before reporting completion, downloads verify received bytes against `Content-Length`, and single-file payloads against the plaintext size from metadata. Truncated transfers fail loudly instead of saving corrupt files.
...
- **Server-side limit gates**: `/download/url/:id`, `/download/:id`, and `/download/blob/:id` return `410` once `dl >= dlimit`; `/download/direct/:id` signs the URL before incrementing the counter so a signing failure cannot burn a download credit.
- **Auth nonce rotation**: the HMAC challenge nonce rotates only after successful authentication (failed/unauthenticated requests are challenged with the current nonce), so concurrent viewers of an encrypted file cannot invalidate each other's in-flight requests.
