...
- **Server-side limit gates**: `/download/url/:id`, `/download/:id`, and `/download/blob/:id` return `410` once `dl >= dlimit`; `/download/direct/:id` signs the URL before incrementing the counter so a signing failure cannot burn a download credit, and signs it for a **size-derived lifetime** (`directDownloadUrlLifetime`, `lib/download-url-lifetime.ts`: transfer time at a 256 KiB/s floor plus an hour, clamped to `[1h, 7d]` — seven days is the SigV4 ceiling on S3 and R2 alike) rather than a flat hour. That route spends the credit at the redirect because a native browser download never tells the page it finished, and the browser's resume-after-drop re-requests the exact signed URL, so a URL that expired mid-transfer turned the resume into a 403 with the credit already gone. `/download/url/:id` keeps the flat hour: the resilient stream behind it refreshes on 403. `/download/complete/:id` re-reads `dlimit` from Redis after incrementing rather than judging against the pre-increment snapshot, so an owner raise landing mid-request cannot cause the file to be destroyed. `/params/:id` clamps `dlimit` to `1..MAX_DOWNLOADS` (integer) exactly as `/upload/url` does at creation — stored verbatim, a huge value makes the gate unreachable, a non-positive value bricks the file, and a float round-trips through Redis as exponential notation.
...
- **Bucket CORS policy.** The browser talks to the bucket directly and reads response headers that only appear when `ExposeHeaders` lists them: `ETag` (read after each multipart part, `api.ts` part completion) and `Content-Range` (read on download range-resume). Required policy: `AllowedMethods` `PUT`/`GET`/`HEAD`, `AllowedOrigins` set to the frontend origin(s), `AllowedHeaders` `*`, `ExposeHeaders` including `ETag` and `Content-Range`. Without it, multipart uploads fail *after* transferring every byte. A hidden `Content-Range` no longer fails a resume: `Headers.get` returns `null` for a header CORS stripped, and a `206` is itself proof the `Range` was honoured (a server that ignores `Range` answers `200`, one that cannot satisfy it answers `416`), while the advertised-length and ECE guards fail closed on a wrong offset anyway — so `createResilientDownloadStream` trusts the status and emits a `download.range-resume` warning instead. Until 15 Sep 2026 the production `bolter` bucket exposed only `ETag`, and every resume of a multi-GB download died on "Range resume mismatch" after the whole transfer had been paid for (BOLTER-FRONTEND-64); a present-but-mismatched `Content-Range` still throws. Full policy in `README.md` → Deployment → Operational requirements.
