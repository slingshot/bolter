...
export class S3Storage {
    ...
    async getSignedDownloadUrl(id: string, filename?: string, expiresIn = 3600): Promise<string> {
        ...
        logger.debug(
            { id, urlLength: url.length, urlPreview: `${url.substring(0, 100)}...` },
    ...
    async getSignedMultipartUploadUrl(
        ...
        logger.debug(
            { id, uploadId, partNumber, expiresIn },
            'Generating signed multipart upload URL',
        );
        const startTime = Date.now();
        ...
        try {
            ...
            if (partNumber === 1 || partNumber % 100 === 0) {
                ...
                logger.debug(
                    { id, uploadId, partNumber, duration },
                    'Signed multipart upload URL generated',
                );
            }
