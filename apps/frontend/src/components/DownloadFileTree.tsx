...
function buildFileTree(files: FileInfo[]): TreeNode {
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < parts.length - 1; i++) {
            ...
            const child = current.children.get(folderName);
            if (!child) {
                break;
            }
            current = child;
...
function TreeView({ node, depth, expandedFolders, onToggleFolder }: TreeViewProps) {
    ...
    return (
        <>
            ...
            {sortedFiles.map((file, index) => (
                // biome-ignore lint/suspicious/noArrayIndexKey: FileInfo has no unique identifier
